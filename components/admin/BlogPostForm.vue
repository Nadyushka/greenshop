...
onMounted(()=> {
  if (id.value) {
    const post = blogPostsData.value.find(post => post.id === id.value)
...
const saveFormData = async () => {
  if (id.value) {
    await plantsStore.savePostChanges(formData)
  } else {
    await plantsStore.addPost(formData)
  }
      ...
      <div class="post__modal-body">
        ...
        <div class="post__row">
          <div>
            ...
            <input v-model="formData.date" class="post__input" placeholder="example: September 12"/>
          </div>
        </div>
        ...
        <div class="post__row">
          <div>
            ...
            <input v-model="formData.length" class="post__input" placeholder="example: Read in 6 minutes"/>
          </div>
        </div>
